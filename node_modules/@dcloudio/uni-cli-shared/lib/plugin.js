...
const pluginRE = /^(uni-|@[\w-]+(\.)?[\w-]+((\/core\/lib)?)\/uni-)/

function resolvePlugins () {
  const pkg = require(path.resolve(process.env.UNI_CLI_CONTEXT, 'package.json'))
  const uniPluginsPkg = fs.readFileSync(path.resolve(process.env.UNI_CLI_CONTEXT, 'node_modules/@dcloudio/core', 'package.json'), 'utf-8');
  return Object.keys(pkg.devDependencies || {})
    .concat(Object.keys(pkg.dependencies || {}))
    .concat(Object.keys(JSON.parse(uniPluginsPkg).uniPliguns || {}))
    .map(id => {
      if (!pluginRE.test(id)) {
        return
      }
